...
from ComponentExtraction import ComponentExtraction
import colorsys

# Create distinct colors using HSV color space
def generate_colors(n_groups):
   return [colorsys.hsv_to_rgb(i/n_groups, 0.8, 0.8) for i in range(n_groups)]
...
class MotorcycleGraph:
    def __init__(self, mesh, vertices, faces, edges, extraordinaryVertices, boundary_vertices, boundary_edges):
        ...
        self.face_component_extraction = None
        self.submeshes = []
    ...
    def constructMotorcycleGraph(self):
        ...
        # extract face components based on the self.motorcycleEdges
        self.face_component_extraction = ComponentExtraction(self.Q, self.E, self.motorcycleEdges)
        print("number of face components: ", len(self.face_component_extraction.face_components))
    ...
    def visualize_motorcycle_edges(self):
        ...
        for edge in self.motorcycleEdges:
            v1_idx, v2_idx = edge
            v1 = self.V[v1_idx] # -1 for especial cases when faces index starts from 1
            v2 = self.V[v2_idx] # -1 for especial cases when faces index starts from 1
            x = [v1[0], v2[0]]
            y = [v1[1], v2[1]]
            z = [v1[2], v2[2]]
    ...
    def visualize_motorcycle_edges_vista(self, mesh):
        ...
        mesh.cell_data['colors'] = np.zeros(mesh.n_cells)
        # colors = generate_colors(len(self.face_component_extraction.face_components))
        ...
        for edge in self.motorcycleEdges:
            v1_idx, v2_idx = edge
            v1 = self.V[v1_idx]  # -1 for indexing
            v2 = self.V[v2_idx]  # -1 for indexing
